...
function daTiro()
    disparo:play()
    local tiro = {
        x  = aviao_14bis.x + aviao_14bis.largura/2 - 2,
        y = aviao_14bis.y,
        largura = 16,
        altura = 16
    }
...
function temColisao(X1,Y1,L1,A1,X2,Y2,A2,L2) 
   return X2 < X1 + L1 and
          X1 < X2+ L2 and 
          Y1 < Y2 + A2 and
          Y2 < Y1 + A1 
...
function removeMeteoros()
    for i = #meteoros, 1, -1 do --diminuir um meteoro no loop
...
function move14bis()
    if love.keyboard.isDown('w') then
        aviao_14bis.y = aviao_14bis.y-aviao_14bis.velocidade
    end
    if love.keyboard.isDown('s') then
        aviao_14bis.y= aviao_14bis.y+aviao_14bis.velocidade
    end
    if love.keyboard.isDown('a') then
        aviao_14bis.x= aviao_14bis.x-aviao_14bis.velocidade
    end
...
function love.load()
    ...
    love.window.setTitle("alura Tutorial")
